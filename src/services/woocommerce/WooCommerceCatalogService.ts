...
export class WooCommerceCatalogService implements CatalogService {
  ...
  async getCategories(): Promise<Category[]> {
    try {
      ...
      return wooCategories.map(wooCategory =>
        CategoryConverter.toBasicCategory(
          CategoryConverter.fromWooCommerce(wooCategory, 'woocommerce')
        )
      );
    } catch (error) {
  ...
  async getProducts(categoryId?: string): Promise<Product[]> {
    try {
      ...
      return wooProducts.map(wooProduct =>
        ProductConverter.toBasicProduct(
          ProductConverter.fromWooCommerce(wooProduct, 'woocommerce')
        )
      );
    } catch (error) {
  ...
  async getProduct(id: string): Promise<Product | undefined> {
    try {
      ...
      return ProductConverter.toBasicProduct(
        ProductConverter.fromWooCommerce(wooProduct, 'woocommerce')
      );
    } catch (error) {
