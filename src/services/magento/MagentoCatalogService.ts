...
export class MagentoCatalogService implements CatalogService {
  ...
  async getProducts(categoryId?: string): Promise<Product[]> {
    try {
      ...
      searchCriteria += '&searchCriteria[filter_groups][1][filters][0][field]=status&searchCriteria[filter_groups][1][filters][0][value]=1&searchCriteria[filter_groups][1][filters][0][condition_type]=eq';
  ...
  private flattenCategories(categories: MagentoCategory[]): MagentoCategory[] {
    ...
    for (const category of categories) {
      if (category.is_active && category.level > 1) { // Skip root category
  ...
  private getProductImage(product: MagentoProduct): string | undefined {
    const imageEntry = product.media_gallery_entries?.find(entry =>
