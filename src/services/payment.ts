import type { Basket, Money } from './types';
...
export class SquarePaymentService implements PaymentService {
  ...
  async promptPin(amount: Money): Promise<{ success: boolean; pinEntered?: boolean; error?: string }> {
    // Simulate PIN entry
...
export class StripePaymentService implements PaymentService {
  constructor(private apiKey: string, private publishableKey?: string) {}
...
export class PaymentServiceFactory {
  static createService(
    type: 'stripe' | 'square' | 'adyen' | 'cash' | 'mock',
    config?: any
  ): PaymentService {
    switch (type) {
      case 'stripe':
