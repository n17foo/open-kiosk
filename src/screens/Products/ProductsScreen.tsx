...
import type { Product, Category, VariantItem, VariantGroup } from '../../services/types';
...
const ProductsScreen: React.FC = () => {
  ...
  const { categories, products, isLoading, getUpgradeById, variantGroups } = useCatalog();
  ...
  const handleToggleVariant = (variant: VariantItem, groupId: string) => {
    ...
    setSelectedVariants(prev => {
      ...
      const currentGroupCount = prev.filter(v => 
        group?.items.some(item => item.id === v.id)
      ).length;
      
      if (currentGroupCount >= maxSelection) {
        const firstGroupVariant = prev.find(v => 
          group?.items.some(item => item.id === v.id)
        );
  ...
  const handleAddToBasket = async () => {
    ...
    await addItem({
      ...
      qty: quantity,
      lineTotal: { amount: lineTotal, currency: 'GBP' },
  ...
  // Get relevant variant groups for the selected product
  const getRelevantVariantGroups = useCallback((product: Product) => {
    ...
    switch (product.categoryId) {
      ...
      case 'womens_clothing':
        relevantGroups.push('size_options', 'color_options', 'material_options');
        break;
      case 'electronics':
        if (product.id === 'wireless_headphones') {
          relevantGroups.push('storage_options', 'color_options', 'headphone_features');
        } else {
          relevantGroups.push('storage_options', 'color_options');
        }
        break;
      case 'accessories':
        if (product.id === 'smartphone_case') {
          relevantGroups.push('color_options', 'phone_compatibility');
        } else if (product.id === 'leather_wallet') {
          relevantGroups.push('color_options', 'leather_type');
        } else {
          relevantGroups.push('color_options');
        }
        break;
      case 'home':
        if (product.id === 'ceramic_mug') {
          relevantGroups.push('color_options', 'mug_customization');
        } else {
          relevantGroups.push('color_options');
        }
        break;
    ...
    return variantGroups.filter(group => relevantGroups.includes(group.id));
  }, [variantGroups]);
    ...
    <View style={styles.screen}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.basketButton} onPress={handleBasketPress}>
          ...
          <Text style={styles.basketTotal}>{formatMoney(basket.total.amount)}</Text>
        </TouchableOpacity>
      </View>
                ...
                >
                  <View style={styles.productImagePlaceholder}>
                    {product.image ? (
                      ...
                      <Text style={styles.productEmoji}>{getCategoryIcon(product.categoryId)}</Text>
                    )}
                  </View>
                  <View style={styles.productInfo}>
                    <Text style={styles.productName} numberOfLines={2}>
              ...
              <>
                  ...
                  <View style={styles.modalProductInfo}>
                    <View style={styles.modalProductImage}>
                      <Text style={styles.modalProductEmoji}>
                        {getCategoryIcon(selectedProduct.categoryId)}
                      </Text>
                    </View>
                    <View style={styles.modalProductDetails}>
                      <Text style={styles.modalProductName}>{selectedProduct.name}</Text>
                      {selectedProduct.description && (
                        <Text style={styles.modalProductDesc}>{selectedProduct.description}</Text>
                      )}
                      <Text style={styles.modalProductPrice}>
                        {formatMoney(selectedProduct.price.amount)}
                      </Text>
                    </View>
                  </View>
                      ...
                      {getRelevantVariantGroups(selectedProduct).map(group => (
                        <View key={group.id} style={styles.optionGroup}>
                          <View style={styles.optionGroupHeader}>
                            ...
                            {group.max && (
                              <Text style={styles.optionGroupMax}>Select up to {group.max}</Text>
                            )}
                          </View>
                                ...
                                >
                                  ...
                                  {addon.price.amount > 0 && (
                                    <Text style={styles.optionPrice}>
                                      +{formatMoney(addon.price.amount)}
                                    </Text>
                                  )}
                ...
                <View style={styles.modalFooter}>
                  ...
                  >
                    ...
                    <Text style={styles.addToBasketPrice}>
