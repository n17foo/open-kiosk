...
export class ProductConverter {
  static fromWooCommerce(wooProduct: WooCommerceProductData, platformType: PlatformType = 'woocommerce'): ExtendedProduct {
  ...
  static fromShopify(shopifyProduct: ShopifyProductData, platformType: PlatformType = 'shopify'): ExtendedProduct {
    ...
    return {
      ...
      categoryIds: shopifyProduct.collections.edges.map(edge => this.extractShopifyId(edge.node.id)),
      ...
      variants: shopifyProduct.variants.edges.map(edge => ({
        ...
        regularPrice: edge.node.compareAtPrice
          ...
          : undefined,
        sku: edge.node.sku,
        options: edge.node.selectedOptions.reduce(
          (acc, option) => {
  ...
  static fromMagento(magentoProduct: MagentoProductData, platformType: PlatformType = 'magento'): ExtendedProduct {
    const currency = 'GBP'; // TODO: Make configurable
    const imageAttribute = magentoProduct.custom_attributes.find(attr => attr.attribute_code === 'image');
    const weightAttribute = magentoProduct.custom_attributes.find(attr => attr.attribute_code === 'weight');
...
export class CategoryConverter {
  static fromWooCommerce(wooCategory: WooCommerceCategoryData, platformType: PlatformType = 'woocommerce'): ExtendedCategory {
    return {
  ...
  static fromShopify(shopifyCollection: ShopifyCollectionData, platformType: PlatformType = 'shopify'): ExtendedCategory {
    return {
  ...
  static fromMagento(magentoCategory: MagentoCategoryData, platformType: PlatformType = 'magento'): ExtendedCategory {
    return {
