    ...
    try {
      ...
      if (categoryId) {
        filteredProducts = products.filter((product: ShopifyProduct) =>
          product.collections.edges.some((edge: any) => this.extractId(edge.node.id) === categoryId)
        );
      }

      return filteredProducts.map((product: ShopifyProduct) => ({
        id: this.extractId(product.id),
        categoryId: product.collections.edges[0] ? this.extractId(product.collections.edges[0].node.id) : 'default',
        name: product.title,
